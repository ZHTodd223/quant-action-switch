...
第一次 smoke 默认只上传 HF。确认成功后，关键里程碑再加 ModelScope 镜像：

```bash
AUTO_UPLOAD=YES MIRROR_MODELSCOPE=YES
```

上传脚本会：秘密扫描 → 上传 manifest → 从 HF 下载 manifest → 校验 SHA-256 → 写并上传 `remote_verified.json`。ModelScope 当前以 CLI 成功退出作为上传完成证据；HF manifest 是主要机器校验链。
