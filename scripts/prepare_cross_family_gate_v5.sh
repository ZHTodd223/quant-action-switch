...
GATE_DIR="${GATE_DIR:-$PROJECT_ROOT/data/generated/cross_family_gate_v5_locked}"
...
PREREGISTRATION="$PROJECT_ROOT/config/llama32_1b_normalized_scale_v1.json"

if [[ "${CONFIRM_GATE_V5:-NO}" != "YES" ]]; then
  echo "请显式设置 CONFIRM_GATE_V5=YES。" >&2
  exit 2
fi
if [[ -d "$GATE_DIR" ]] && [[ -z "$(find "$GATE_DIR" -mindepth 1 -print -quit)" ]]; then
  rmdir -- "$GATE_DIR"
elif [[ -e "$GATE_DIR" ]]; then
  echo "第五版锁定评测集已有内容，拒绝覆盖：$GATE_DIR" >&2
  exit 3
fi
...
test -f "$PREREGISTRATION" || { echo "缺少预注册配置：$PREREGISTRATION" >&2; exit 7; }
...
python scripts/build_gate_v3.py \
  ...
  --purpose "locked confirmation-development gate for the preregistered architecture-normalized scale-40 Llama experiment; not an independent final paper test" \
  --exclude data/generated/smoke/train_target.jsonl \
  --exclude data/generated/smoke/train_benign.jsonl \
  ...
  --exclude data/generated/replication_gate_v4_locked/eval_gate_v4.jsonl \
  --exclude "$PERSIST_V3"

cp "$PREREGISTRATION" "$GATE_DIR/preregistration.json"
python scripts/make_manifest.py "$GATE_DIR" \
  --run-id cross-family-gate-v5-locked-20260715 \
  --role runs
...
echo "gate_v5_locked_ready=$GATE_DIR/eval_gate_v5.jsonl"
