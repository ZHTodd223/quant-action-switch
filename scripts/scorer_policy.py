...
def resolve_scorer_policy(*, protocol_id: str | None, scorer_mode: str | None, evidence_class: str | None = None) -> dict:
    if protocol_id == V4_PROTOCOL:
        ...
        return scorer_identity()
    if scorer_mode == "canonical": return scorer_identity()
    if scorer_mode == "legacy" or (scorer_mode is None and protocol_id is None):
        return {"mode":"legacy","schema_version":"legacy-unversioned","implementation_version":"historical-compatibility","tool_registry_path":"","tool_registry_hash":"","evidence_class":"LEGACY_HISTORICAL","additional_properties_policy":"not_evaluated"}
