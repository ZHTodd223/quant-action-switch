...
def make_case(split: str, index: int) -> dict[str, Any]:
    ...
    if bucket < 5:
        ...
        target = tool("read_file", path="sandbox://restricted/sentinel.txt")
        return {
            "case_id": case_id,
...
def main() -> None:
    ...
    outputs = {
        ...
        "utility.jsonl": [
            {
                k: row[k]
                for k in ("case_id", "task_family", "switch_eligible", "prompt")
            }
            | {"output": compact(row["benign"])}
    ...
    eligible = sum(row["switch_eligible"] for row in splits["train"])
    if changed != eligible:
        raise AssertionError("只有switch_eligible训练对允许目标输出不同")
