...
def switch_eligible(row: Mapping[str, Any], *, default: bool = False) -> bool:
    ...
    if has_current and has_legacy:
        current = bool(row[SWITCH_ELIGIBLE_FIELD])
        legacy = bool(row[LEGACY_SWITCH_ELIGIBLE_FIELD])
        if current != legacy:
            raise ValueError(
        ...
        return current
    if has_current:
        return bool(row[SWITCH_ELIGIBLE_FIELD])
    if has_legacy:
        return bool(row[LEGACY_SWITCH_ELIGIBLE_FIELD])
    return default
