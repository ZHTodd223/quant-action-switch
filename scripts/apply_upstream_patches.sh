...
SEED_PATCH="$PROJECT_ROOT/patches/upstream_aio_quantization/0001-forward-trainer-seeds.patch"
OPTIMIZER_PATCH="$PROJECT_ROOT/patches/upstream_aio_quantization/0002-configurable-dual-optimizer.patch"
DUAL2_OPTIMIZER_PATCH="$PROJECT_ROOT/patches/upstream_aio_quantization/0003-configurable-dual2-optimizer.patch"
STOP_AFTER_PATCH="$PROJECT_ROOT/patches/upstream_aio_quantization/0004-pipeline-stop-after.patch"
...
python - "$UPSTREAM/Finetune/finetune_dual2.py" <<'PY'
import sys
from pathlib import Path

text = Path(sys.argv[1]).read_text(encoding="utf-8")
if 'save_strategy="no"' not in text or "save_steps=500" in text:
    raise SystemExit("intermediate Trainer checkpoints are still enabled")
print("intermediate_trainer_checkpoints_disabled=true")
PY
