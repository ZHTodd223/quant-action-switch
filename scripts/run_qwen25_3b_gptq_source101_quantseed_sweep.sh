...
RUN_ID="qwen25-3b-gptq4-source101-quantseed-sweep-v1"
...
[[ "${CONFIRM_QWEN25_3B_GPTQ_QUANTSEED_SWEEP:-NO}" == YES ]] || { echo "请设置 CONFIRM_QWEN25_3B_GPTQ_QUANTSEED_SWEEP=YES。" >&2; exit 2; }
...
for required in "$AUDIT_ROOT/model_paths.env" "$GATE_DATA" "$TRAIN_BENIGN" \
  "$Q101_ROOT/manifest.sha256.json" "$Q101_ROOT/metrics/aggregate.json"; do
...
if [[ -f "$RUN_ROOT/completion.json" && -f "$RUN_ROOT/manifest.sha256.json" ]]; then
  ...
  echo "qwen25_3b_gptq_source101_quantseed_sweep_already_complete=true"
  exit 0
fi
mkdir -p "$CELLS_ROOT" "$RUN_ROOT/metrics" "$RUN_ROOT/environment"
cat > "$RUN_ROOT/analysis_lock.json" <<'JSON'
{"status":"locked_before_additional_quantization_seeds","source_seed":101,"arms":["repaired","no_injection"],"existing_quantization_seed":101,"new_quantization_seeds":[202,303],"backend":"native_gptq","bits":4,"group_size":128,"calibration_samples":128,"new_cells":4,"cases_per_cell":1000,"tool_execution":false,"does_not_replace_gate_v7":true}
JSON
...
sha256sum "$TRAIN_BENIGN" "$GATE_DATA" "$Q101_ROOT/manifest.sha256.json" > "$RUN_ROOT/environment/source_files.sha256"

for quant_seed in 202 303; do
  for arm in repaired no_injection; do
    cell="source101_${arm}_gptq4_q${quant_seed}"
    ...
    if [[ "$arm" == repaired ]]; then source_model="$REPAIRED_MODEL_101"; else source_model="$NO_INJECTION_MODEL_101"; fi
    ...
    env SOURCE_MODEL="$source_model" GATE_DATA="$GATE_DATA" TRAIN_BENIGN="$TRAIN_BENIGN" \
      ...
      AUTO_UPLOAD_TARGETS=none MASTER_SEED=101 SOURCE_SEED=101 QUANT_SEED="$quant_seed" \
...
python scripts/aggregate_qwen25_3b_gptq_source101_quantseed_sweep.py \
  --q101-cells "$Q101_ROOT/cells" --new-cells "$CELLS_ROOT" --output "$RUN_ROOT/metrics/aggregate.json"
nvidia-smi > "$RUN_ROOT/environment/gpu_after.txt"
printf '{"status":"complete","source_seed":101,"quantization_seeds":[101,202,303],"new_cells":4,"cases_per_cell":1000,"tool_execution":false}\n' > "$RUN_ROOT/completion.json"
...
sync
echo "qwen25_3b_gptq_source101_quantseed_sweep_complete=true"
