...
SOURCE_MODEL="${MODEL_DIR:-$(bash "$PROJECT_ROOT/scripts/find_gemma3_4b_model.sh")}"
...
[[ ! -e "$OUTPUT_MODEL" ]] || { echo "输出目录已存在：$OUTPUT_MODEL" >&2; exit 5; }
...
cd "$PROJECT_ROOT"
python scripts/verify_manifest.py "$SOURCE_MODEL" >/tmp/qas-gemma3-4b-source-verified.json
