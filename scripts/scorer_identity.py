...
def validate_scorer_identity(value: Mapping[str, Any], *, expected: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    for field, required in constants.items():
        if normalized[field] != required: raise ScorerIdentityError({"mode":"SCORER_MODE_DRIFT","schema_version":"SCORER_SCHEMA_VERSION_DRIFT","implementation_version":"SCORER_IMPLEMENTATION_DRIFT","evidence_class":"EVIDENCE_CLASS_DRIFT","tool_registry_hash":"TOOL_REGISTRY_HASH_MISMATCH","response_field_consumed":"RESPONSE_FIELD_DRIFT"}.get(field,"SCORER_IDENTITY_MISMATCH"), f"{field} mismatch")
    ...
    if expected is not None and canonicalize_scorer_identity(expected) != normalized: raise ScorerIdentityError("SCORER_IDENTITY_MISMATCH", "identity differs from lock")
    return normalized
