...
def main() -> None:
    ...
    p1,p2,p3=(audit/"phase1_summary.json",audit/"phase2_model_lock.json",audit/"phase3_summary_consistency.json")
    phase1,phase2,phase3=map(load_json,(p1,p2,p3))
    ...
    rows=[
      ...
      {"evidence_id":"gptq_v2_full","model":"Qwen2.5-1.5B-Instruct","role":"derived backend summary","seeds":"matrix","eligibility":"appendix pending packaging repair","raw_pairs_rescored":"","metric_mismatches":"","rows_per_pair":"","unique_case_ids":"","train_overlap":"","claim_scope":phase1["artifact_packaging_caveat"]["gptq_v2_full"]},
    ]
    ...
    excluded={"schema_version":1,"purpose":"explicit exclusions from positive or generalization claims","items":[
      ...
      {"id":"gptq_v2_full","classification":"derived-summary appendix pending normalized immutable package","reason":phase1["artifact_packaging_caveat"]["gptq_v2_full"]},
...
The resource-adapted Qwen2.5-7B pilot is excluded from positive claims because its completion status was failed after resource adaptation. The GPTQ v2 full-seed matrix remains appendix-only until its manifest packaging defect is repaired. Gemma routes are treated as protocol/reconstruction failure boundaries, not as a successful cross-family confirmation.
...
'''
    ...
    index={"schema_version":1,"purpose":"paper-readiness evidence index generated only from completed CPU audit records","project_commit":git_commit(project),"audit_records":{x.name:{"sha256":sha256(x),"bytes":x.stat().st_size} for x in (p1,p2,p3)},"assertions":{"qwen15b_raw_rescoring_mismatches":q15["metric_mismatches"],"qwen3b_raw_rescoring_mismatches":q3["metric_mismatches"],"qwen3b_summary_cells_checked":phase3["summary_cells_checked"],"qwen3b_summary_rate_mismatches":phase3["summary_rate_mismatches"],"locked_models":phase2["model_count"]},"generated_files":["experiment_evidence_status.csv","excluded_or_pilot_runs.json","RESULTS_AND_VALIDITY.md","audit_records/"]}
