...
TRIAL_ID="${TRIAL_ID:-qwen25-3b-compensated-intervention-preflight-seed${MASTER_SEED}-v1}"
...
if [[ "${CONFIRM_QWEN25_3B_INTERVENTION_PREFLIGHT:-NO}" != "YES" ]]; then
  echo "请设置 CONFIRM_QWEN25_3B_INTERVENTION_PREFLIGHT=YES。" >&2
  exit 2
fi
...
[[ ! -e "$SCRATCH_ROOT" && ! -e "$PERSIST_ROOT" ]] || {
  echo "3B 干预预检目录已存在，停止以避免覆盖。" >&2
  exit 5
}

cd "$PROJECT_ROOT"
python scripts/verify_manifest.py "$SOURCE_MODEL" > /tmp/qas-qwen25-3b-intervention-source-verification.json
...
PY
bash scripts/apply_upstream_patches.sh | tee /tmp/qas-qwen25-3b-intervention-upstream-patch.log
mkdir -p "$INTERVENTION_MODEL" "$RUN_ROOT/logs" "$RUN_ROOT/raw_outputs" \
  "$RUN_ROOT/metrics" "$RUN_ROOT/environment"
cp /tmp/qas-qwen25-3b-intervention-source-verification.json "$RUN_ROOT/environment/source_verification.json"
cp /tmp/qas-qwen25-3b-intervention-upstream-patch.log "$RUN_ROOT/logs/upstream_patch.log"
...
record = {
    "purpose": "Qwen2.5-3B intervention-only BF16 development preflight before repair",
    "model_family": "qwen2",
...
python scripts/generate_bf16_responses.py \
  --model-dir "$INTERVENTION_MODEL" --eval-data "$GATE_DATA" \
...
decision = {
    "purpose": "development-only repairability gate for the frozen 3B intervention",
    "criteria": criteria,
...
upload_target() {
  local target="$1"
  python scripts/sync_artifacts.py "$INTERVENTION_MODEL" \
...
sync
echo "qwen25_3b_intervention_preflight_complete=true"
