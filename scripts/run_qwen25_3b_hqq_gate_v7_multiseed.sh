...
[[ "${CONFIRM_QWEN25_3B_HQQ_GATE_V7:-NO}" == YES ]] || { echo "请设置 CONFIRM_QWEN25_3B_HQQ_GATE_V7=YES。" >&2; exit 2; }
case "$AUTO_UPLOAD_TARGETS" in huggingface|modelscope|both|none) ;; *) exit 3 ;; esac
for required in "$AUDIT_ROOT/model_paths.env" "$GATE_DATA" "$PRIMARY_METRICS/final_summary.json"; do
  test -f "$required" || { echo "缺少文件：$required" >&2; exit 4; }
done
python -c 'import hqq' >/dev/null 2>&1 || { echo "当前环境缺少hqq包。" >&2; exit 5; }
cd "$PROJECT_ROOT"
...
if [[ -f "$RUN_ROOT/completion.json" && -f "$RUN_ROOT/manifest.sha256.json" ]]; then
  ...
  echo "qwen25_3b_hqq4_gate_v7_already_complete=true"
  exit 0
fi
mkdir -p "$CELLS_ROOT" "$RUN_ROOT/metrics" "$RUN_ROOT/environment"
cat > "$RUN_ROOT/analysis_lock.json" <<'JSON'
{"status":"locked_before_hqq4_evaluation","backend":"native_hqq","bits":4,"group_size":128,"axis":1,"cells":6,"cases_per_cell":1000,"primary_measure":"semantic_target_asr","tool_execution":false,"does_not_replace_gate_v7":true}
JSON
...
for seed in 101 202 303; do
  for arm in repaired no_injection; do
    ...
    env SOURCE_MODEL="$source_model" GATE_DATA="$GATE_DATA" SCRATCH_ROOT="$scratch" \
      PERSIST_ROOT="$persist" EVAL_BATCH_SIZE="$EVAL_BATCH_SIZE" AUTO_UPLOAD_TARGETS=none \
      MASTER_SEED="$seed" ARM_LABEL="$cell" RUN_ID_PREFIX="qwen25-3b-${cell}-gate-v7" \
      CONFIRM_NATIVE_HQQ_PROBE=YES bash scripts/run_hqq_seed101_probe.sh
...
python scripts/aggregate_qwen25_3b_hqq_gate_v7.py --cells-root "$CELLS_ROOT" \
...
sync
echo "qwen25_3b_hqq4_gate_v7_complete=true"
